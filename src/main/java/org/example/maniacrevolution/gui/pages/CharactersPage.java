...
public class CharactersPage extends GuidePage {
    ...
    private static void buildExtraSections() {
        ...
        {
            List<ExtraSection> s = new ArrayList<>();
            s.add(new ExtraHeader("§5§lХранитель кошмаров"));
            s.add(new ExtraText("§7Хранитель кошмаров - медленный маньяк контроля. Он не играет через быстрые атаки: его основная сила в давлении взглядом, рассудке выживших и кошмарных испытаниях."));
            s.add(new ExtraText("§7В форме Хранителя нельзя атаковать обычным ударом и нельзя бегать. Он ходит медленнее привычных персонажей, поэтому ему важно заранее выбирать позицию, держать линию взгляда и вынуждать выживших ошибаться."));

            s.add(new ExtraSpacer(6));
            s.add(new ExtraHeader("§d§lВзгляд и рассудок"));
            s.add(new ExtraText("§7Когда Хранитель держит выжившего во взгляде, у цели начинает падать ментальное здоровье. Это не обычные HP, а отдельная шкала рассудка, которая появляется только когда в матче есть Хранитель."));
            s.add(new ExtraText("§7Чем дольше выживший находится под давлением взгляда, тем ближе он к кошмарному испытанию. Выжившим нужно разрывать обзор, прятаться за препятствиями и не давать Хранителю держать стабильную линию."));
            s.add(new ExtraText("§7У Хранителя есть способность 'Концентрированный кошмар': она резко снимает часть рассудка у цели во взгляде и вызывает первородный страх у персонажа. Это инструмент добивания рассудка, а не обычная атака."));

            s.add(new ExtraSpacer(6));
            s.add(new ExtraHeader("§c§lКокон"));
            s.add(new ExtraText("§7Когда рассудок выжившего ломается, игрок попадает в кошмарное испытание, а в обычном мире появляется кокон. Кокон нельзя сдвинуть, а обычные удары по нему не решают проблему."));
            s.add(new ExtraText("§7Кокон имеет запас прочности. Над ним отображается красная полоска: чем меньше прочности осталось, тем больше красная часть заменяется бордовой."));
            s.add(new ExtraText("§7Если кокон разрушить вовремя, выживший возвращается из испытания. Если команда игнорирует кокон, игрок остаётся один на один с испытанием и рискует получить тяжёлые последствия."));
            s.add(new ExtraImageCaption(
                    "guide/mechanics/characters/nightmare/cocoon.png", 420, 220,
                    "Кокон"));

            s.add(new ExtraSpacer(6));
            s.add(new ExtraHeader("§b§lИгла пробуждения"));
            s.add(new ExtraText("§7Игла пробуждения - ключевой предмет против кокона. Она выдаётся ограниченно: примерно одна игла на трёх игроков, поэтому носители иглы должны играть аккуратно и не рисковать без причины."));
            s.add(new ExtraText("§7Чтобы нанести урон кокону, игрок с иглой запускает мини-игру. Нужно провести линию по ломаному маршруту. Если точность ниже требуемого порога, урон не засчитывается."));
            s.add(new ExtraText("§7Если рука соскочила, линия тоже уходит с маршрута, и это снижает точность. Не пытайтесь прожимать иглу в панике: промах тратит время команды и может стоить спасения."));
            s.add(new ExtraImageCaption(
                    "guide/mechanics/characters/nightmare/needle_minigame.png", 420, 220,
                    "Мини-игра иглы"));

            s.add(new ExtraSpacer(6));
            s.add(new ExtraHeader("§6§lИспытания"));
